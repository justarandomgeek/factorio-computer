...
* `ScalarGen.blueprint.lua`: A Foreman Blueprint Script to generate the Scalar Pick/Return mechanism for the current game's signal list. The local bus generated by MaskGen is, from left to right, Blue,Yellow,Cyan,Green.
* `MaskGen.blueprint.lua`: A Foreman Blueprint Script to generate the Vector pairwise Multiply/Add instructions (Ops 61 and 62). This is an optional part of the machine implementing these instructions. The local bus generated by MaskGen is, from left to right, Orange,Cyan,Green.
