...
The Scalar Pick & Return mechanism allows operating on individual signals from registers. There are two Pick channels, R1.s1 and R2.s2, and one Return channel R2.sd, selected by the corresponding signals in the current opcode. Because the signals available will vary depending on what mods are installed, this module is generated by a script, ScalarGen. ScalarGen is executed by pasting it into Foreman's import window as if it were a blueprint string, and will add a blueprint named ScalarGen to your list. It will also produce a file `scalarmap.lua` in your script-output directory listing the numberic mappings for the assembly it has generated, which can be must be used when compiling programs for your computer.
